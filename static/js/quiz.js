document.addEventListener("DOMContentLoaded", function () {
  ...
  let confidenceThreshold = 0.9; // Increased threshold to 90% for stricter validation
  let socket = null;
  ...
  function initializeSocket() {
    ...
    if (typeof io === "undefined") {
      console.error(
        "Socket.IO is not loaded. Make sure to include the socket.io client script."
      );
      feedbackMessage.textContent =
        "Error: Socket.IO not loaded. Please refresh the page.";
    ...
    socket.on("asl-result", (data) => {
      ...
      if (isLocal) {
        ...
        if (text === targetLetter && confidence >= confidenceThreshold) {
          ...
          feedbackMessage.textContent = `Almost there! Keep signing "${targetLetter}" clearly. (${Math.round(
            confidence * 100
          )}% confident)`;
          ...
          // No valid prediction
          feedbackMessage.textContent =
            "No valid sign detected. Please try again.";
          feedbackMessage.className = "feedback-message feedback-warning";
        }
      }
    });
  ...
  function handleCorrectPrediction() {
    ...
    letterScore.textContent = Math.max(
      1,
      Math.floor(secondsRemaining / 12) + 1
    );
